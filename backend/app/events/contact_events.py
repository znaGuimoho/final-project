...
from app.services.redis_db import save_message
...
def register_socketio_handelers(app, templates, get_db, app_sio, sio: AsyncServer):
    ...
    async def join_room(sid, data):
        ...
        if not room:
            await sio.emit(
                "error_message", {"message": "No room code provided"}, to=sid
            )
            return
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                if not room_data:
                    await sio.emit(
                        "error_message", {"message": "Invalid room code"}, to=sid
                    )
                    return
                ...
                if current_user_id not in [user_id, hoster_id]:
                    ...
                    print(
                        f"{Fore.RED}User {current_user_id} unauthorized for room {room}{Style.RESET_ALL}"
                    )
                    return
                ...
                print(
                    f"{Fore.GREEN}User {current_user_id} joined room {room}{Style.RESET_ALL}"
                )
    ...
    async def send_message(sid, data):
        ...
        if not room or not message:
            await sio.emit(
                "error_message", {"message": "Missing room or message"}, to=sid
            )
            return
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                if not room_data:
                    await sio.emit(
                        "error_message", {"message": "Room not found"}, to=sid
                    )
                    return
                ...
                print(
                    f"{Fore.GREEN}Message sent in room {room} by {name} ({sender_role}){Style.RESET_ALL}"
                )
                ...
                await sio.emit(
                    "error_message", {"message": "Failed to send message"}, to=sid
                )
    ...
    async def disconnect(sid):
        ...
        print(
            f"{Fore.GREEN}Client {sid} (user: {user_id}) disconnected{Style.RESET_ALL}"
        )
